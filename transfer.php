<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        if($numexistsrows > 0 && $acno != $transferacno && $amount > 0 && $amount <= $balance)
        {
            ...
            $debitsql =  "UPDATE `users` SET `balance` = '$debitAmount' WHERE `acno` = $acno";
            $debitsqlresult = mysqli_query($conn, $debitsql);
            

            // BENEFICIARY'S PART
            $sql = "SELECT balance FROM `users` WHERE acno = '$transferacno'";
            $result = mysqli_query($conn, $sql);
            $num = mysqli_num_rows($result);

            if ($num == 1) {
                while($row = mysqli_fetch_assoc($result)) {
                $beneficiaryBalance = $row["balance"];
                }
            }

            $creditAmount = $beneficiaryBalance + $amount;

            $creditsql = "UPDATE `users` SET `balance` = '$creditAmount' WHERE `acno` = $transferacno";
            $creditsqlresult = mysqli_query($conn, $creditsql);
